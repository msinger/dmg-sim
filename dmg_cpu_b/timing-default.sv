...
package dmg_timing;
	...
	parameter bit nodelay = 0;

	// Wire parasitics
	parameter real Rprime_wire    = 0.0005;   // ohm/µm
	parameter real Cprime_wire    = 0.13e-15; // F/µm
	...
	parameter real kR_nmos_ref    = 2.93e4;   // ohm*µm for Lref
	parameter real gamma_pn       = 1.98;     // PMOS = gamma * NMOS resistance

	parameter real Cint_stage_F   = 0.5e-15;  // F intrinsic capacitance added after each stage

	parameter real Cgate_per_um_F = 0.60e-15; // F capacitance per transistor gate width

	// Scale for converting Electric VLSI layout units to µm
	parameter real L_unit         = 0.454;
	...
	function automatic realtime tpd_z(realtime T_plh = 0.0s, realtime T_phl = 0.0s, realtime T_min = T_Z_min);
		...
		return (0.3 * T_pd) > T_min ? (0.3 * T_pd) : T_min;
	endfunction
