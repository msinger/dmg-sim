...
module dmg_dffr #(
	...
	always_latch if (!clk || !r_n) ff  <= d & r_n;
	always_latch if (clk || !r_n)  q   <= ff & r_n;
	assign                         q_n  = !q;
	...
	specify
		...
		(clk *> q)   = (T_fall_not1 + T_rise_buf + T_fall_nand + T_rise_q,
		                T_fall_not1 + T_rise_not2 + T_fall_buf + T_rise_nand + T_fall_q);
		(r_n *> q)   = (T_fall_nand + T_rise_q, T_rise_nand + T_fall_q);
		(clk *> q_n) = (T_fall_not1 + T_rise_not2 + T_fall_buf + T_rise_nand + T_fall_not3 + T_rise_q_n,
		                T_fall_not1 + T_rise_buf + T_fall_nand + T_rise_not3 + T_fall_q_n);
