...
module dmg_pad_xtal #(
	...
	localparam realtime T_rise_mux_d1 = tpd_elmore(130, R_pmos_ohm(72*L_unit), C_gate_F(245*L_unit));
	localparam realtime T_fall_mux_d1 = tpd_elmore(130, R_nmos_ohm(62*L_unit), C_gate_F(245*L_unit));
	...
	localparam realtime T_rise_out_pad = tpd_elmore(L_out_pad, R_pmos_ohm(136*L_unit), C_gate_F(W_gate_out_pad));
	localparam realtime T_fall_out_pad = tpd_elmore(L_out_pad, R_nmos_ohm( 55*L_unit), C_gate_F(W_gate_out_pad));
	assign #(T_rise_out_pad, T_fall_out_pad) out_pad = !clk;

	localparam realtime T_rise_clk_n = tpd_elmore(L_clk_n, R_pmos_ohm(30*L_unit), C_gate_F(W_gate_clk_n));
	localparam realtime T_fall_clk_n = tpd_elmore(L_clk_n, R_nmos_ohm(24*L_unit), C_gate_F(W_gate_clk_n));
